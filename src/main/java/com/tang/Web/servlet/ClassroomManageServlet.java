...
@WebServlet(urlPatterns = "/ClassroomManageServlet")
public class ClassroomManageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            classRoomList = classRoomService.getClassRoomListByPage(sqlRow.toString());
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        PageBean<ClassRoom> pageBean=new PageBean<>(currentPage,pageSize,totalRows,classRoomList);
        ServletContext servletContext = request.getServletContext();
        servletContext.setAttribute("pageBeanClassroom",pageBean);
        request.setAttribute("pageBeanClassroom",pageBean);
