...
@WebServlet(urlPatterns = "/TeacherSearchServlet")
public class TeacherSearchServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        StringBuffer sqlRow=new StringBuffer("Select course_id courseId,course_name courseName,course_hour courseHour,course_time courseTime,user_id userId,team_id teamId,classroom_id classroomId from course where team_id=?");
        sqlRow.append(" limit ").append(startRow).append(",").append(pageSize);
        //把所有课程信息查询出来
        List<Course> courseList= null;
        try {
            courseList = courseService.getCourseListByPage(sqlRow.toString(),Integer.valueOf(teamId));
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        PageBean<Course> pageBean=new PageBean<>(currentPage,pageSize,totalRows,courseList);
        ServletContext servletContext = request.getServletContext();
        servletContext.setAttribute("pageBean1",pageBean);
        request.setAttribute("pageBean1",pageBean);
        request.getRequestDispatcher("teacher/teacherSearch.jsp").forward(request,response);
    }
}
